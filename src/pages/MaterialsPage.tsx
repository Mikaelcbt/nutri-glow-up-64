...
import { FileText, Download, File, Table } from 'lucide-react';
    ...
    <AppLayout>
      <AnimatedPage>
        <div className="max-w-5xl mx-auto px-4 py-8">
          <motion.h1 variants={fadeInUp} initial="initial" animate="animate" className="font-display text-3xl font-semibold text-foreground mb-8">Materiais</motion.h1>
            ...
            <motion.div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4" variants={staggerContainer} initial="initial" animate="animate">
              {materials.map(m => (
                <motion.div
                  key={m.id}
                  variants={fadeInUp}
                  className="rounded-2xl border border-border bg-card p-6 shadow-card flex flex-col gap-4 transition-all duration-300 hover:shadow-soft hover:-translate-y-1 hover:scale-[1.02]"
                >
                  ...
                  <Button asChild variant="outline" size="sm" className="border-primary text-primary hover:bg-accent active:scale-[0.97] transition-transform">
