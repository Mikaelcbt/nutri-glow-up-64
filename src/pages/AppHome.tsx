...
import { ChevronLeft, ChevronRight, Play, BookOpen, Layers, Clock, Lock, CheckCircle } from 'lucide-react';
...
export default function AppHome() {
  ...
  const [habitsToday, setHabitsToday] = useState({ completed: 0, total: 0 });
  ...
  const exploreCardRefs = useRef<(HTMLDivElement | null)[]>([]);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // ── Progresso: UMA query para todos os produtos acessíveis ──────────────
      // Coleta todos os lesson IDs por produto primeiro
      const lessonIdsByProduct = new Map<string, string[]>();
      const allAccessibleLessonIds: string[] = [];

      for (const p of accessibleProducts) {
        const mods = p.modules || [];
        const ids = mods.flatMap((m: any) => (m.lessons || []).map((l: any) => l.id as string));
        lessonIdsByProduct.set(p.id, ids);
        allAccessibleLessonIds.push(...ids);
      }
      ...
      if (allAccessibleLessonIds.length > 0) {
        ...
        completedSet = new Set(allProgress?.map(p => p.lesson_id as string) ?? []);
      }

      // Monta progresso por produto sem queries adicionais
      const progMap: Record<string, number> = {};
      for (const p of accessibleProducts) {
        const ids = lessonIdsByProduct.get(p.id) ?? [];
        progMap[p.id] = ids.length > 0
          ? (ids.filter(id => completedSet.has(id)).length / ids.length) * 100
          : 0;
      }
      setProgressMap(progMap);

      const featured = accessibleProducts[0] || products[0];
      ...
      // Próxima aula — reutiliza completedSet, sem query extra
      if (hasAccess) {
        const featuredLessonIds = lessonIdsByProduct.get(featured.id) ?? [];
        const nextId = featuredLessonIds.find(id => !completedSet.has(id));
        if (nextId) {
          const { data: lessonData } = await supabase
            .from('lessons').select('id, titulo').eq('id', nextId).maybeSingle();
          if (lessonData) setNextLesson({ id: lessonData.id, title: lessonData.titulo });
        }
      }

      // Hábitos e desafio ativo em paralelo
      const today = new Date().toISOString().split('T')[0];
      const [habitsRes, habitsRecordRes, challengeRes] = await Promise.all([
        supabase.from('habitos').select('id').eq('ativo', true).limit(50),
        supabase.from('habitos_registro').select('habito_id').eq('user_id', user.id).eq('data', today).eq('concluido', true).limit(50),
        supabase.from('desafios').select('id, titulo').eq('is_active', true).order('created_at', { ascending: false }).limit(1),
      ]);
      setHabitsToday({ total: habitsRes.data?.length ?? 0, completed: habitsRecordRes.data?.length ?? 0 });
      if (challengeRes.data?.[0]) {
        setActiveChallenge({ id: challengeRes.data[0].id, title: challengeRes.data[0].titulo });
      }
  ...
  const scrollExplore = (d: 'left' | 'right') => {
    const container = exploreRef.current;
    if (!container) return;
    container.scrollBy({ left: d === 'left' ? -240 : 240, behavior: 'smooth' });
  };
  ...
  const handleExploreScroll = useCallback(() => {
    ...
    setExploreIndex(Math.max(0, Math.min(idx, allProducts.length - 1)));
  }, [allProducts.length]);

  const scrollToExploreCard = (index: number) => {
    const container = exploreRef.current;
    if (!container) return;
    container.scrollTo({ left: index * 232, behavior: 'smooth' });
  ...
  return (
    <AppLayout>
      <AnimatedPage>
        ...
        {/* ── Greeting ── */}
        <motion.div className="px-4 pt-5 md:px-16 md:pt-7 space-y-0.5" variants={fadeInUp}>
          <p className="text-[10px] uppercase tracking-[0.18em] text-primary font-semibold">{getTodayLabel()}</p>
          <h2 className="text-xl md:text-2xl font-semibold text-foreground">
            {getGreeting()}, <span className="text-foreground">{profile?.nome_completo?.split(' ')[0] || 'Usuária'}</span>
          </h2>
        </motion.div>

        {/* ── Today Card ── */}
        <TodayCard
          habitsCompleted={habitsToday.completed}
        ...
        {featuredProduct ? (
          <section ref={heroRef} className="relative h-[60vh] md:h-[80vh] w-full overflow-hidden">
            ...
            <div className="relative flex h-full items-center px-4 md:px-16">
              <motion.div className="max-w-xl space-y-4 md:space-y-5" variants={staggerContainer} initial="initial" animate="animate">
                <motion.div variants={fadeInUp} className="flex items-center gap-3">
                  <div className="h-px w-6 bg-primary" />
                  <span className="text-[10px] md:text-xs font-bold uppercase tracking-[0.2em] text-primary">
                    {featuredHasAccess ? 'Programa em Destaque' : 'Disponível'}
                  </span>
                  <div className="h-px w-6 bg-primary" />
                </motion.div>

                <motion.h1 variants={fadeInUp} className="font-display text-4xl leading-tight text-foreground md:text-7xl font-semibold">{featuredProduct.nome}</motion.h1>
                ...
                <motion.div variants={fadeInUp} className="flex items-center gap-2 text-xs md:text-sm text-muted-foreground">
                  <span className="flex items-center gap-1.5"><Layers className="h-3.5 w-3.5" /> {animatedModules} módulos</span>
                  <span className="text-border">|</span>
                  <span className="flex items-center gap-1.5"><BookOpen className="h-3.5 w-3.5" /> {animatedLessons} aulas</span>
                </motion.div>
                ...
                <motion.div variants={fadeInUp} className="flex flex-col sm:flex-row gap-3">
                  {featuredHasAccess ? (
                    <>
                      ...
                      <Button asChild variant="outline" size="lg" className="h-12 px-6 md:px-8 text-sm md:text-base font-semibold border-primary/40 text-primary hover:bg-accent transition-all duration-300">
            ...
            {featuredHasAccess && (
              <motion.div className="absolute bottom-0 left-0 right-0 px-6 py-4 md:px-16" style={{ background: 'linear-gradient(to top, hsl(var(--background)/0.95), transparent)' }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}>
                <div className="flex items-center gap-4">
                  <span className="text-xs text-muted-foreground font-medium whitespace-nowrap">Seu progresso</span>
                  ...
                  <span className="text-xs text-primary font-bold tabular-nums">{Math.round(productProgress)}%</span>
                </div>
              </motion.div>
            )}
        ...
        <motion.section className="px-4 py-8 md:px-16 md:py-12" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
          <div className="mb-5 md:mb-6 flex items-center justify-between">
            <h2 className="font-display text-2xl md:text-3xl font-semibold text-foreground border-l-[3px] border-primary pl-3">Meus Programas</h2>
            {myPrograms.length > 1 && (
              <div className="flex gap-2">
                <button onClick={() => scrollMyProgs('left')} aria-label="Programa anterior" className="rounded-full border border-border/60 p-2 text-foreground hover:bg-accent hover:border-primary/30 transition-all duration-200"><ChevronLeft className="h-4 w-4" /></button>
                <button onClick={() => scrollMyProgs('right')} aria-label="Próximo programa" className="rounded-full border border-border/60 p-2 text-foreground hover:bg-accent hover:border-primary/30 transition-all duration-200"><ChevronRight className="h-4 w-4" /></button>
              </div>
            )}
          </div>
            ...
            <div ref={myProgsRef} className="flex gap-4 md:gap-5 overflow-x-auto scroll-smooth pb-4 snap-x snap-mandatory md:snap-none" style={{ scrollbarWidth: 'none' }}>
              {myPrograms.map((p, i) => {
                ...
                return (
                  ...
                  >
                    <Link to={`/app/programa/${p.slug}`} className="group block bg-card border border-border/50 rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-1.5 hover:shadow-green-glow hover:border-primary/25">
                      ...
                      <div className="p-4 space-y-3">
                        ...
                        <div className="space-y-1.5">
                          <div className="flex justify-between text-xs">
                            ...
                            <span className="text-primary font-bold tabular-nums">{Math.round(prog)}%</span>
                          </div>
                          <Progress value={prog} className="h-2" />
                        </div>
                        <Button size="sm" className="w-full btn-ripple bg-gradient-to-r from-primary to-[hsl(142_72%_37%)] shadow-sm hover:shadow-green-glow transition-all duration-300 gap-2">
        ...
        >
          <div className="px-4 md:px-16 mb-5 md:mb-6 flex items-center justify-between">
            <h2 className="font-display text-2xl md:text-3xl font-semibold text-foreground border-l-[3px] border-primary pl-3">Explore os Programas</h2>
            {allProducts.length > 1 && (
              <div className="flex gap-2">
                <button
                  onClick={() => scrollExplore('left')}
                  className="rounded-full border border-border/60 p-2 text-foreground hover:bg-accent hover:border-primary/30 transition-all duration-200"
                  aria-label="Rolar programas para a esquerda"
                >
                ...
                <button
                  ...
                  className="rounded-full border border-border/60 p-2 text-foreground hover:bg-accent hover:border-primary/30 transition-all duration-200"
                  aria-label="Rolar programas para a direita"
                >
          ...
          <div
            ...
            style={{ scrollbarWidth: 'none', paddingLeft: 'max(1rem, calc((100vw - 1280px) / 2 + 4rem))', paddingRight: '1rem' }}
          >
            {allProducts.map((p, i) => {
              const hasAccess = isAdmin || !!accessMap[p.id];
              const CardWrapper = hasAccess ? Link : 'button' as any;
              const wrapperProps = hasAccess
                ? { to: `/app/programa/${p.slug}` }
                : { onClick: () => setRequestModal(p.nome), type: 'button' as const };

              return (
                <motion.div
                  key={p.id}
                  ref={el => { exploreCardRefs.current[i] = el; }}
                ...
                >
                  <CardWrapper
                    {...wrapperProps}
                    className="group block relative overflow-hidden transition-transform duration-300 hover:scale-[1.03] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary w-full text-left"
                  ...
                  >
                    ...
                    <div
                      ...
                      style={{ background: 'linear-gradient(to top, rgba(0,0,0,0.96) 0%, rgba(0,0,0,0.55) 45%, rgba(0,0,0,0.1) 70%, transparent 100%)' }}
                    />
                    ...
                    <div className="absolute inset-x-0 bottom-0 p-4 space-y-2">
                      <h3
                        className="text-white font-display font-bold uppercase leading-tight line-clamp-2"
                        style={{ fontSize: '15px', letterSpacing: '0.02em' }}
                      >
                        {p.nome}
                      </h3>
                      <div className="flex items-center justify-between">
                        <span className="text-white/40 text-[9px] uppercase tracking-[0.18em] font-semibold">
                          JP NutriCare
                        </span>
                        {hasAccess ? (
                          <span className="flex items-center justify-center w-5 h-5 rounded-full bg-primary/80">
          ...
          {allProducts.length > 1 && (
            <div className="flex items-center justify-center gap-1.5 mt-4">
              {allProducts.map((_, i) => (
                <button
                  key={i}
                  onClick={() => scrollToExploreCard(i)}
                  className="transition-all duration-300 rounded-full"
                  style={{
                    width: i === exploreIndex ? '20px' : '6px',
                    height: '6px',
                    background: i === exploreIndex ? 'hsl(var(--primary))' : 'hsl(var(--muted-foreground)/0.35)',
                  }}
        ...
        {/* ── Footer ── */}
        <footer className="border-t border-border/40 px-4 py-6 md:px-16 mt-4">
